...
const studentSchema = z.object({
  ...
  id: z.string().trim().min(4, "Enter a valid student ID").max(30),
...
const initials = (name: string) =>
  name
    ...
    .slice(0, 2)
    .toUpperCase();
const performance = (score: number) =>
        ...
        items.map((s) =>
          s.id === editingId
            ? {
                ...
                ...recordValues,
                ...photoValues,
                grade: gradeFor(values.score),
                status: statusFor(values.score),
              }
            : s,
        ),
      );
...
function LoginScreen({ onLogin }: { onLogin: () => void }) {
  ...
  function handleSubmit(event: React.FormEvent) {
    ...
    if (username === "ambition" && password === "ambition0508") {
      setError("");
        ...
        <div className="relative grid w-full max-w-4xl overflow-hidden rounded-2xl border border-border bg-card shadow-2xl backdrop-blur-xl lg:grid-cols-2">
          ...
          <div className="p-8 sm:p-10">
            ...
            <form className="mt-8 grid gap-4" onSubmit={handleSubmit}>
              ...
              {error && (
                <div className="flex items-center gap-2 rounded-md border border-danger/30 bg-danger/10 px-3 py-2 text-xs font-medium text-danger">
                  {error}
                </div>
              )}
...
function Dashboard({
  ...
  const metrics = [
    ...
    [
      "Avg. attendance",
      `${(students.reduce((a, s) => a + s.attendance, 0) / students.length).toFixed(2)}%`,
      CalendarDays,
      "emerald",
    ],
    [
  ...
  return (
    <>
      <header className="sticky top-0 z-30 border-b border-border bg-header/90 backdrop-blur-xl print:hidden">
        <div className="mx-auto flex max-w-[1500px] flex-wrap items-center gap-4 px-5 py-4 lg:flex-nowrap lg:px-8">
          <Brand />
          <div className="order-3 flex w-full flex-1 items-center gap-2 rounded-md border border-border bg-surface-raised px-3 lg:order-none lg:ml-8 lg:max-w-xl">
      ...
      <div className="mx-auto max-w-[1500px] px-5 py-9 lg:px-8 lg:py-14">
        ...
        <section className="mt-10 flex flex-col gap-6 border-y border-border py-6 lg:flex-row lg:items-start">
          <div>
...
function MonthlyCards({ exams }: { exams: Exam[] }) {
  return (
      ...
      <DialogContent key={key} className="max-h-[92vh] max-w-3xl overflow-y-auto bg-card p-0">
        ...
        >
          ...
          {[
            ...
            ["attendance", "Attendance (%)", student?.attendance, "number"],
          ...
          <label className="form-field">
            <span>Teacher</span>
            <select name="teacher" defaultValue={student?.teacher ?? "Suraj Bist"}>
          ...
          <label className="form-field">
            ...
            <select name="timing" defaultValue={student?.timing ?? "8-10 AM"}>
